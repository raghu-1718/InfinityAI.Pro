...
class InfinityAISystemVerifier:
    ...
    def _load_config(self):
        """Loads configuration from infrastructure/config.json"""
        try:
            with open("infrastructure/config.json", "r") as f:
                return json.load(f)
        except FileNotFoundError:
            raise FileNotFoundError("Critical Error: infrastructure/config.json not found. Cannot proceed with verification.")
