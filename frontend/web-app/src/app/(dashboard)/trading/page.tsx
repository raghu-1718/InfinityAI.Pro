...
export default function TradingPage() {
    ...
    const { data: accountData, error: accountError, isError: isAccountError } = useUserAccount();
  ...
  const funds = accountData?.funds?.availableBalance || 0;
    ...
    return (
        <div className="flex flex-col items-center min-h-[calc(100vh-4rem)] p-6 gap-8 max-w-7xl mx-auto w-full">
            {!dhanConnected && (
                <DhanConnectPrompt onConnect={() => window.location.href = '/dashboard/settings'} />
            )}
            {isAccountError && (
                <div className="bg-red-900 text-red-100 p-3 rounded mb-4 w-full text-center">
                    <strong>Account Error:</strong> {accountError?.message || 'Failed to load account data.'}
                </div>
            )}
      ...
      <div className="text-center space-y-4 w-full py-8">
        <h1 className="text-5xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-indigo-500 to-purple-400 tracking-tighter flex items-center justify-center gap-4 neon-text">
           <Activity className={cn("w-12 h-12 md:w-16 md:h-16", isEngineRunning ? "text-green-400 animate-pulse drop-shadow-[0_0_15px_rgba(74,222,128,0.5)]" : "text-slate-600")} />
        ...
        <div className="max-w-3xl mx-auto mt-6 transition-all duration-500 hover:scale-[1.02]">
          ...
          <div className={cn("lg:col-span-8 glass-card p-6 md:p-8 space-y-8 relative overflow-hidden group", isEngineRunning && "opacity-60 pointer-events-none grayscale-[0.8]")}>
             ...
             <div className="flex items-center gap-4 border-b border-white/10 pb-6">
                 ...
                 <div>
                    ...
                    <p className="text-slate-400">Define risk parameters and asset allocation</p>
                 </div>
             </div>
             ...
             <div className="space-y-8">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     <div className="space-y-3">
                         <Label className="text-slate-300 text-sm font-semibold uppercase tracking-wider">Asset Class</Label>
                         <Select value={assetClass} onValueChange={setAssetClass} disabled={isEngineRunning}>
                             ...
                             <SelectContent className="glass border-white/10">
                                 <SelectItem value="NIFTY" className="focus:bg-indigo-500/20">NIFTY 50</SelectItem>
                                 <SelectItem value="BANKNIFTY" className="focus:bg-indigo-500/20">BANK NIFTY</SelectItem>
                                 <SelectItem value="FINNIFTY" className="focus:bg-indigo-500/20">FIN NIFTY</SelectItem>
                                 <SelectItem value="multi_asset" className="focus:bg-purple-500/20 font-bold">MULTI-ASSET (Unified)</SelectItem>
                                 <SelectItem value="CRUDEOIL" className="focus:bg-indigo-500/20">CRUDE OIL (Comm)</SelectItem>
                                 <SelectItem value="GOLD" className="focus:bg-indigo-500/20">GOLD (Comm)</SelectItem>
                             </SelectContent>
                         </Select>
                     </div>
                     <div className="space-y-3">
                         <div className="flex justify-between">
                            <Label className="text-slate-300 text-sm font-semibold uppercase tracking-wider">Deploy Capital</Label>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-12 pt-4">
                     <div className="space-y-6">
                         <div className="flex justify-between items-end">
                             ...
                             <span className="font-mono text-2xl font-bold text-rose-500 drop-shadow-[0_0_8px_rgba(244,63,94,0.4)]">{riskPerTrade.toFixed(1)}%</span>
                         </div>
                         <Slider 
                            value={[riskPerTrade]} 
                     ...
                     <div className="space-y-6">
                         <div className="flex justify-between items-end">
                             ...
                             <span className="font-mono text-2xl font-bold text-emerald-400 drop-shadow-[0_0_8px_rgba(52,211,153,0.4)]">{targetProfit.toFixed(1)}%</span>
                         </div>
                         <Slider 
                            value={[targetProfit]} 
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-white/5">
                    <div className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/5 transition-all hover:bg-white/10">
                        <div className="space-y-1">
                            <Label className="font-semibold text-white">Trailing Stop</Label>
                            <p className="text-xs text-slate-400">Lock profits effectively</p>
                        </div>
                        <Switch checked={isTrailing} onCheckedChange={setIsTrailing} disabled={isEngineRunning} className="data-[state=checked]:bg-indigo-500"/>
                    </div>
                    
                    <div className={cn("flex items-center justify-between p-4 rounded-xl border transition-all duration-300", isContinuous ? "border-purple-500/50 bg-purple-500/20 shadow-[0_0_20px_-5px_rgba(168,85,247,0.3)]" : "border-white/5 bg-white/5 hover:bg-white/10")}>
                        <div className="space-y-1">
                            <div className="flex items-center gap-2">
                                <Label className={cn("font-semibold", isContinuous ? "text-purple-300" : "text-white")}>Continuous Mode</Label>
                                {isContinuous && <InfinityIcon className="w-4 h-4 text-purple-400 animate-spin-slow" />}
                            </div>
                            <p className="text-xs text-slate-400">Run until manual stop</p>
                        </div>
                        <Switch checked={isContinuous} onCheckedChange={setIsContinuous} disabled={isEngineRunning} className="data-[state=checked]:bg-purple-600"/>
                    </div>
                 </div>
             </div>
          </div>
          ...
          <div className="lg:col-span-4 space-y-6">
              ...
              {/* Start/Stop Button */}
              <div className={cn("p-[1px] rounded-3xl bg-gradient-to-b shadow-2xl transition-all duration-500", isEngineRunning ? "from-rose-500 via-red-500 to-orange-500 shadow-[0_0_50px_-10px_rgba(239,68,68,0.5)]" : "from-emerald-400 via-green-500 to-teal-500 shadow-[0_0_50px_-10px_rgba(16,185,129,0.3)]")}>
                <Button 
                    onClick={handleStartStop}
                    disabled={isLoading || isKillSwitchActive}
                    className={cn("w-full h-40 rounded-[22px] flex flex-col items-center justify-center gap-3 bg-black/90 hover:bg-black/80 transition-all border-none relative overflow-hidden group")}
                >
                    <div className={cn("absolute inset-0 opacity-20 bg-[url('/noise.png')] mix-blend-overlay")} />
                    <div className={cn("absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 bg-gradient-to-b", isEngineRunning ? "from-red-500/20 to-transparent" : "from-green-500/20 to-transparent")} />
                    
                    <Power className={cn("w-14 h-14 transition-all duration-300 z-10", isEngineRunning ? "text-red-500 animate-pulse drop-shadow-[0_0_10px_red]" : "text-green-500 group-hover:drop-shadow-[0_0_10px_#22c55e]")} />
                    
                    <div className="text-center z-10">
                        <span className={cn("text-4xl font-black tracking-[0.2em] block neon-text", isEngineRunning ? "text-red-100" : "text-green-100")}>
                            {isEngineRunning ? "STOP" : "START"}
                        </span>
                        <span className={cn("text-xs uppercase tracking-[0.3em] opacity-70 font-mono mt-1 block", isEngineRunning ? "text-red-300" : "text-green-300")}>
              ...
              <div className={cn("transition-all duration-500 overflow-hidden", isEngineRunning ? "max-h-64 opacity-100" : "max-h-0 opacity-0")}>
                   <div className="glass-card p-6 border-l-4 border-l-indigo-500 bg-gradient-to-r from-indigo-900/40 to-black/40">
                       <h3 className="text-xs uppercase font-bold tracking-widest text-indigo-400 mb-4 flex items-center gap-2">
                           <Activity className="w-3 h-3 animate-bounce" /> Active Configuration
                       </h3>
                       <div className="space-y-4 font-mono">
                           <div className="flex justify-between items-center text-sm border-b border-white/5 pb-2">
                               <span className="text-slate-400">Asset</span>
                               <span className="font-bold text-white text-lg">{assetClass}</span>
                           </div>
                           <div className="flex justify-between items-center text-sm border-b border-white/5 pb-2">
                               <span className="text-slate-400">Target</span>
                               <span className="font-bold text-emerald-400 text-lg">+{targetProfit}%</span>
                           </div>
                           <div className="flex justify-between items-center text-sm">
                               <span className="text-slate-400">Mode</span>
                               <Badge variant="outline" className={cn("border-purple-500 text-purple-300", isContinuous && "bg-purple-500/10")}>
             ...
             <div className="glass-card p-5 border border-red-900/30 bg-gradient-to-br from-red-950/30 to-black/50">
                 <div className="flex items-center justify-between gap-4">
                     <div className="flex items-center gap-4">
                         <div className={cn("p-2 rounded-lg bg-red-900/20 border border-red-900/50", isKillSwitchActive && "animate-pulse border-red-500 text-red-500")}>
                             <ShieldAlert className={cn("w-6 h-6", isKillSwitchActive ? "text-red-500" : "text-red-800")} />
                         </div>
                         <div>
                             <h3 className="font-bold text-red-100">Kill Switch</h3>
                             <p className="text-xs text-red-400/70 uppercase tracking-wider">Emergency Halt</p>
                         </div>
                     </div>
                     <Switch 
            ...
            <div className="glass-card border-t-4 border-t-blue-500/50">
                <div className="p-6 border-b border-white/5 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-400">Real-time decision logs from Engine A</p>
                    </div>
                </div>
                <div className="p-6">
