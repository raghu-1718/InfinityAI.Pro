"""
InfinityAI.Pro — Institutional Options Tri-Model Walk-Forward Training Engine
==============================================================================
"""
...
# Add path to engine-a mlops
sys.path.insert(0, os.path.abspath("backend/engine-a/src/mlops"))
from ensemble_definition import OptionsEnsembleModel

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = logging.getLogger("OptionsModelTrainer")

PROJECT_ID = os.getenv("GOOGLE_CLOUD_PROJECT", "project-841b7f97-5ee3-4fbe-920")
BUCKET_NAME = "infinity-ai-models-vault"
TABLE_ID = f"{PROJECT_ID}.market_data.options_training_features"
...
def evaluate_fold(model: OptionsEnsembleModel, X: np.ndarray, y: np.ndarray, returns: np.ndarray) -> Dict[str, float]:
    """Computes options quantitative performance metrics for a split."""
...
def run_options_walk_forward_training() -> Dict[str, Any]:
    """Runs Options Walk-Forward Validation and registers production ensemble."""
    bq_client = bigquery.Client(project=PROJECT_ID)
    db = firestore.Client(project=PROJECT_ID)
    storage_client = storage.Client(project=PROJECT_ID)

    logger.info("📥 Pulling Options features from BigQuery...")
    query = f"SELECT * FROM `{TABLE_ID}` ORDER BY bar_date ASC"
    df = bq_client.query(query).to_dataframe()
    df["bar_date"] = pd.to_datetime(df["bar_date"])
    logger.info(f"   • Loaded {len(df):,} total options training records ({df['bar_date'].min().date()} -> {df['bar_date'].max().date()}).")
    ...
    fold_results = []
    logger.info("\n🧪 Starting 3-Fold Walk-Forward Cross-Validation (Options)...")
    ...
    logger.info("\n🏆 Training Final Production Options Ensemble on Full Dataset...")
    X_full = df[FEATURE_COLS].to_numpy(dtype=np.float32)
    y_full = np.asarray(df["label_win"].values, dtype=np.int32)
    prod_model = OptionsEnsembleModel()
    prod_model.fit(X_full, y_full)

    version = f"v{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
    os.makedirs("trained_models/options", exist_ok=True)
    local_artifact_path = f"trained_models/options/options_ensemble_{version}.joblib"
    local_meta_path = f"trained_models/options/options_ensemble_{version}_metadata.json"
    ...
    metadata = {
        ...
        "gcs_artifact_uri": f"gs://{BUCKET_NAME}/options/options_ensemble_{version}.joblib"
    }
    ...
    try:
        ...
        blob_model = bucket.blob(f"options/options_ensemble_{version}.joblib")
        blob_model.upload_from_filename(local_artifact_path)
        blob_meta = bucket.blob(f"options/options_ensemble_{version}_metadata.json")
        blob_meta.upload_from_filename(local_meta_path)
        logger.info(f"☁️ Uploaded Options Ensemble to GCS: {metadata['gcs_artifact_uri']}")
    except Exception as e:
        logger.error(f"Failed uploading to GCS: {e}")

    try:
        db.collection("model_training_runs").document(f"OPTIONS_{version}").set(metadata)
        logger.info(f"🔥 Registered training run in Firestore `model_training_runs/OPTIONS_{version}`")
    except Exception as e:
        logger.error(f"Failed logging to Firestore: {e}")

    logger.info("🎉 Options Model Training & Walk-Forward Cross-Validation Completed!")
    return metadata

if __name__ == "__main__":
    run_options_walk_forward_training()
