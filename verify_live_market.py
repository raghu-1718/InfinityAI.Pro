...
def verify_live_market():
    ...
    while True:
        try:
            ...
            if latest_ts:
                lag = now - latest_ts
                status = "✅ ACTIVE" if lag < timedelta(minutes=MAX_DELAY_MINUTES) else f"⚠️ DELAYED (Lag: {str(lag).split('.')[0]})"
                print(f"[{now.strftime('%H:%M:%S')}] Status: {status} | Total Rows: {total_rows} | Last Tick: {latest_ts.strftime('%H:%M:%S UTC')}")
            else:
