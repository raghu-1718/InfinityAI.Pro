...
class YahooHistoricalClient:
    ...
    async def upload_to_gcs(
        ...
        """
        Save CSV locally (upload separately via PowerShell/gcloud)
        ...
        try:
            ...
            import os
            local_dir = f"data/yahoo_historical/{symbol}"
            ...
            logger.info(f"✅ Saved {symbol}/{interval}/{period} to {local_path}")
            return True

        except Exception as e:
            logger.error(f"❌ Error saving {symbol} {interval} {period}: {str(e)}")
            return False
