...
export interface AISignalComponents {
  yoloAnalysis?: {
    pattern: string;
    confidence: number;
...
export class AISignalGenerator {
   ...
   */
  async generateBTCSignal(btcPrice: number, chartImage?: Buffer): Promise<BTCOptionsSignal> {
    ...
    const components: AISignalComponents = {};
   ...
   */
  private async analyzeChartWithYOLO(chartImage?: Buffer): Promise<AISignalComponents["yoloAnalysis"]> {
    if (!chartImage || !this.runpodUrl) {
      return undefined;
    }

    try {
      const response = await axios.post(`${this.runpodUrl}/yolo/analyze`, chartImage, {
   ...
   */
  private async analyzeNewsWithWhisper(): Promise<AISignalComponents["whisperSentiment"]> {
    if (!this.runpodUrl) {
      return undefined;
    }
    ...
    try {
      ...
      const response = await axios.post(`${this.runpodUrl}/whisper/sentiment`, {
   ...
   */
  private async analyzeMarketWithLLM(btcPrice: number): Promise<AISignalComponents["llmAnalysis"]> {
    try {
      ...
      if (this.ollamaUrl) {
        ...
        response = await axios.post(`${this.ollamaUrl}/api/generate`, {
          model: "llama3.2",
          prompt,
          stream: false
        });
   ...
   */
  private combineAISignals(components: AISignalComponents): {
    direction: "bull" | "bear";
    confidence: number;
    reasoning: string;
  } {
   ...
   */
  private async getCryptoNewsHeadlines(): Promise<string[]> {
    try {
   ...
   */
  private buildLLMPrompt(btcPrice: number, marketData: any): string {
...
SUMMARY: [one sentence summary]`;
   ...
   */
  private generateMockLLMAnalysis(btcPrice: number, marketData: any): string {
