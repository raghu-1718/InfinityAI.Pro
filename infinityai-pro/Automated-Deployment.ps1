# Automated Deployment Script for InfinityAI Engines
param(
...
# Function to assume deployment role
function Set-DeploymentCredentials {
    ...
    try {
        ...
        if ($roleCreds -and $roleCreds.Credentials) {
            ...
            Write-Host "✅ Assumed identity: $($identity.Arn)" -ForegroundColor Green
    ...
    foreach ($engine in $awsEngines) {
        ...
        try {
            ...
            if ($existingService.services -and $existingService.services[0].status -ne "INACTIVE") {
                ...
                # Get default subnets and security group (you may need to adjust these)
...
function Show-DeploymentSummary {
    ...
    Write-Host "• Configure DNS records in Namecheap" -ForegroundColor Yellow
    Write-Host "• Deploy Engine A to Azure Container Instances" -ForegroundColor Yellow  
    Write-Host "• Deploy Engine B to Google Cloud Run" -ForegroundColor Yellow
    Write-Host "• Set up load balancers and SSL certificates" -ForegroundColor Yellow
    Write-Host "• Perform end-to-end integration testing" -ForegroundColor Yellow
}
...
function Start-Deployment {
    ...
    # Step 1: Assume deployment role
    if (-not (Set-DeploymentCredentials $ExternalId)) {
        Write-Host "❌ Failed to set deployment credentials" -ForegroundColor Red
        exit 1
    }
    
    # Step 2: Check and build Docker images if needed
    if (-not $SkipBuild) {
