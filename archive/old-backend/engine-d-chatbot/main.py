...
class ChatbotService:
    def __init__(self):
        ...
        self.engines = {
            ...
            'engine_b': {
                'name': 'AI/ML Service',
                'url': os.getenv('ENGINE_B_URL', 'https://engine-b-ai-ml-prod-bprmddefsa-uc.a.run.app'),
                'description': 'Advanced AI models and predictions'
            },
            'engine_c': {
                'name': 'Trade Execution Service',
                'url': os.getenv('ENGINE_C_URL', 'https://engine-c-prod-bprmddefsa-uc.a.run.app'),
        ...
        # Initialize GCP Secret Manager client (if available)
        try:
    ...
    def store_dhan_credentials(self, user_id: str, credentials: DhanCredentials) -> dict:
        """Store Dhan credentials securely in GCP Secret Manager"""
        try:
            ...
            project_id = os.getenv('GCP_PROJECT_ID', 'after-yesterday-473512-k3')
            ...
            # Try to add secret version (update existing secret)
            try:
                from google.cloud import secretmanager
    ...
    def get_dhan_credentials(self, user_id: str) -> Optional[dict]:
        """Retrieve Dhan credentials from GCP Secret Manager"""
        try:
            if not self.secrets_client:
            ...
            response = self.secrets_client.access_secret_version(request={"name": secret_path})
            credentials = json.loads(response.payload.data.decode('UTF-8'))
            logger.info(f"Retrieved Dhan credentials for user {user_id}")
            return credentials
            
        except Exception as e:
            logger.warning(f"No Dhan credentials found for user {user_id}: {e}")
            return None
